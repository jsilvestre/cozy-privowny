...
task 'lint', 'Lint the code of the application', (opts) ->
    exec "coffeelint -f coffeelint.json *.coffee", (err, stdout, stderr) ->
        util.print err if err
        util.print stderr if stderr
        util.print stdout if stdout
    exec "coffeelint -r -f coffeelint.json server/", (err, stdout, stderr) ->
        util.print err if err
        util.print stderr if stderr
        util.print stdout if stdout
